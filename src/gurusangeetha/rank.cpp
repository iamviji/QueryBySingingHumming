...
SModel* LoadModel (char* refMelody, RefType refType, float*& pFeaBuf, int& nFeaLen)
{
	...
	if (refType == RefType_Csv)
	{
		...
		if (csv.is_open ())
		{
			...
			std::cout << "number of notes: " << nNotes << "\n";
			csv.clear ();
			csv.seekg(0, std::ios::beg);
			while (csv.good ())
			{
				...
				if (i != 0)
				{
					...
					SQBHModels->sNotes[i-1].fNoteDuration = atof(tk3.c_str ());
					std::cout << SQBHModels->sNotes[i-1].fNoteValue << "," << SQBHModels->sNotes[i-1].fNoteDuration << "\n";
				} else
				{
					std::cout << "skiping header\n";
				}
	...
	else if (refType == RefType_Midi)
	{
		...
		for(i=0;i<myQBHModel.notes.size();i++)
		{
			...
			std::cout << SQBHModels->sNotes[i].fNoteValue << "," << SQBHModels->sNotes[i].fNoteDuration << "\n";
		}
...
/* rank query */
int GetRank(char* refMelody, RefType refType,char* szWav, bool isTestWav)
{
	int i=0;
	float *pFeaBufT = NULL;
	int nFeaLenT = 0;	
...
//	if (isRefWav == isTestWav && isRefWav == true)
	...
	std::cout <<"SimilarityScore=" << myEMDResStru->fScore << ",SimilarityScale=" << myEMDResStru->fScale << ",Mid="<<myEMDResStru->nModelID << ",PID="<< myEMDResStru->nPhraseID<<"\n";
